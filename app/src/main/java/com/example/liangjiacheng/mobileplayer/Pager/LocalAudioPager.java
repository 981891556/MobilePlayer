...
import com.example.liangjiacheng.mobileplayer.Activity.AudioPlayerActivity;
 ...
 * 这个类的作用是：本地音频的页面,这个代码大部分是复制电影的那个Pager
 */
...
//                context.startActivity(intent);
                /**3.传递列表数据  如果是对象需要序列化  */
                Intent intent = new Intent(context, AudioPlayerActivity.class);/**调用自己定义的播放器*/
//                Bundle bundle = new Bundle();
//                bundle.putSerializable("videolist",mediaItems);
//                intent.putExtras(bundle);
                intent.putExtra("position", position);/**跳过页面携带名称为"position"的数据*/
                context.startActivity(intent);
    ...
    private Handler handler = new Handler() {  //使用Handler是因为在子线程中不能设置适配器
        ...
        public void handleMessage(Message msg) {
            ...
            if (mediaItems != null && mediaItems.size() > 0) {
                //有数据
                //设置适配器，也就是显示内容
                videoPagerAdapter = new VideoPagerAdapter(context, mediaItems, false);//看Adapter的代码参数
